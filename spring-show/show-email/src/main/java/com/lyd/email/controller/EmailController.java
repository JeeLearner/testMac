package com.lyd.email.controller;
...
import com.lyd.email.domain.EmailMessage;
import com.lyd.email.receivemail.ReceiveMailInter;
import com.lyd.email.receivemail.ReceiveQQMailImpl;
...
public class EmailController {
	...
	public Json sendmail(HttpServletRequest request, HttpServletResponse response) {
		Json j = new Json();
		SendEmailInter sendmail = new SendQQEmailImpl();
		...
		if (!DataTypeUtil.checkString(toEmail)) {
			j.setMsg("发送失败，请输入邮箱");
			j.setSuccess(false);
			return j;
		}
